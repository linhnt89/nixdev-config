...
- Firstmate tool pins: axi CLIs in `firstmate/node-tools/package.json` (+ lockfile) — bumped by the Dependabot npm lane for `/firstmate/node-tools`, then validated by `scripts/check.sh`; no-mistakes/herdr as SRI-pinned release assets and crew-watch as a pinned source build (commit + source/cargo hashes) in `lib/firstmate.nix`, and treehouse as a tag-pinned flake input (`flake.nix`) — these move by deliberate edit/ref bump (docs/updates.md). crew-watch is an optional read-only human diagnostic sidecar (TUI over firstmate state files; `--fm-home`, `--once`) — never a daemon or a supervision/wake/ack component (docs/firstmate.md). `scripts/check.sh` builds the firstmate devShell and both firstmate profile variants (with/without the herdr backend) on every run and asserts the crew-watch package/profile contract.
